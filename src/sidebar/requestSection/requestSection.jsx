...
import { UserRoundPlus, UserX, HeartHandshake, X } from 'lucide-react';
...
const Requests = ({ setOpenRequest, setNewRequest }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 left-0 w-full h-full flex justify-center items-center z-50 bg-black/70 backdrop-blur-sm bg-opacity-50"
    >
      <div className=" overflow-auto max-sm:pt-10 max-sm:px-2 bg-gray-900 rounded-lg shadow-lg p-4 max-sm:w-full max-sm:h-full w-96 h-2/3">
        <span className="absolute max-sm:top-0 top-4 hover:scale-110  rounded-lg hover:text-white py-1 hover max-sm:right-0 right-2 text-gray-200 cursor-pointer" onClick={closeCard}>
          <X/>
        </span>
        <div className=" text-slate-500">
          <div className='text-center text-lg mb-4 bg-gray-800 text-slate-300 shadow-md'> Requests</div>
          {
            ...
            {requestsRecieved.length === 0 ? (
              ...
              <div>
                {requestsRecieved.map((request) => (
                  <div key={request._id} className=" bg-gray-700 text-slate-300 px-2 mb-2 rounded-md flex justify-between items-center py-2">
                    <span>{request.name.toUpperCase()}</span>
                    {request.isGroup && <span className='text-blue-400 text-[12px] px-2 rounded-xl '>Group Invite</span>}
